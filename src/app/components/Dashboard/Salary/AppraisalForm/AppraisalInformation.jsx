...
const AppraisalInformation = () => {
  ...
  useEffect(() => {
    const fetchLeaveData = async () => {
      try {
        const response = await fetch(`/api/dashboard/appraisal?id=${id}`);
...
if(userLoggedIn){
  ...
  const handleCancelLeave = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/dashboard/appraisal", {
        method: "PUT",
  ...
  return (
    <>
      <Wrapper className="p-5 bg-white rounded-[10px] flex flex-col gap-[15px] w-full relative">
        {leaves?.ExpectedSalary && <H2>Expected Salary: {leaves?.ExpectedSalary}</H2>}
